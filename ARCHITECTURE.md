...
| `/login.html` | [login.html](login.html) | User navigation (redirected here by [app.js:186](app.js:186), [app.js:391](app.js:391), [app.js:431](app.js:431), [app.js:551](app.js:551), and [tasking.js](tasking.js) when no valid session is present). Also the landing page for Supabase magic-link redirects (`emailRedirectTo: window.location.origin + '/login.html'` at [login.html:333](login.html:333)). Submits the OTP form by calling Supabase Auth directly — see "External Services". |
...
| [worker.js](netlify/functions/worker.js) | `/.netlify/functions/worker` | Any (handler ignores method) | Two invocation paths: (1) Netlify scheduled function on cron `0 */4 * * *` (every 4 hours) — see [netlify.toml:36-38](netlify.toml:36). (2) Manually called via HTTP by the "Fetch & triage" button in the tasking SPA ([tasking.js:1314](tasking.js:1314)). Pulls up to 5 oldest `pending` rows, runs each through the triage pipeline, writes results. |
...
| **Supabase Auth (magic-link redirect)** | [/login.html](login.html) (GET, browser navigation) | URL fragment (`access_token` + `refresh_token`); not validated server-side. [app.js:373-385](app.js:373) parses the hash and stashes the session in `localStorage`. | Not really a service-to-server call — it's the user-agent following a redirect after Supabase emails them a sign-in link. Included here because the redirect is configured (`emailRedirectTo`, [login.html:333](login.html:333)) and the page handles the resulting URL. |
...
| **Supabase Auth** at `${SUPABASE_URL}/auth/v1/*` | [auth.js](netlify/functions/auth.js), [_lib/auth.js](netlify/functions/_lib/auth.js), [_lib/routes/admin.js](netlify/functions/_lib/routes/admin.js), [triage.js](netlify/functions/triage.js); also directly from [login.html:294](login.html:294) (`/auth/v1/otp` for magic-link send) and [app.js:115](app.js:115) (`/auth/v1/token?grant_type=refresh_token`). | JWT verification, user creation via `/auth/v1/admin/users`, profile-email enrichment, token refresh, OTP send. |
